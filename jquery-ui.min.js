!function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/version.js", ["jquery"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.ui = $.ui || {},
    $.ui.version = "1.13.1"
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/data", ["jquery", "./version"], factory) : factory(jQuery)
}(function($) {
    "use strict";
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/disable-selection", ["jquery", "./version"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.fn.extend({
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/jquery-var-for-color.js", ["jquery", "./version"], factory) : factory(jQuery)
}(function($) {}),
function(root, factory) {
    "function" == typeof define && define.amd ? define("ui/vendor/jquery-color/jquery.color.js", ["jquery"], factory) : "object" == typeof exports ? module.exports = factory(require("jquery")) : factory(root.jQuery)
}(this, function(jQuery, undefined) {
    var colors, class2type = {}, toString = class2type.toString, rplusequals = /^([\-+])=\s*(\d+\.?\d*)/, stringParsers = [{
    ...
    function clamp(value, prop, allowEmpty) {
        ...
        isNaN(value) ? prop.def : type.mod ? (value + type.mod) % type.mod : Math.min(type.max, Math.max(0, value)))
    }
    function stringParse(string) {
        ...
        each(stringParsers, function(_i, parser) {
            var match = parser.re.exec(string)
              , match = match && parser.parse(match)
              , parser = parser.space || "rgba";
            if (match)
                return match = inst[parser](match),
                inst[spaces[parser].cache] = match[spaces[parser].cache],
                rgba = inst._rgba = match._rgba,
                !1
        }),
    ...
    (color.fn = jQuery.extend(color.prototype, {
        parse: function(red, green, blue, alpha) {
            ...
            this) : "object" === type ? (each(spaces, red instanceof color ? function(_spaceName, space) {
        ...
        transition: function(other, distance) {
            var other = (end = color(other))._space()
              , space = spaces[other]
    ...
    spaces.hsla.to = function(rgba) {
        ...
        var r = rgba[0] / 255
          ...
          , add = max + min
          , l = .5 * add
          , min = min === max ? 0 : r === max ? 60 * (g - b) / diff + 360 : g === max ? 60 * (b - r) / diff + 120 : 60 * (r - g) / diff + 240
          , max = 0 == diff ? 0 : l <= .5 ? diff / add : diff / (2 - add);
        return [Math.round(min) % 360, max, l, null == rgba ? 1 : rgba]
    }
    ,
    spaces.hsla.from = function(hsla) {
        ...
        var h = hsla[0] / 360
          ...
          , s = l <= .5 ? l * (1 + s) : l + s - l * s
          , l = 2 * l - s;
        return [Math.round(255 * hue2rgb(l, s, h + 1 / 3)), Math.round(255 * hue2rgb(l, s, h)), Math.round(255 * hue2rgb(l, s, h - 1 / 3)), hsla]
    }
    ,
    each(spaces, function(spaceName, space) {
        ...
        color.fn[spaceName] = function(value) {
            ...
            var ret, type = getType(value), arr = "array" === type || "object" === type ? value : arguments, local = this[cache].slice();
            return each(props, function(key, prop) {
                key = arr["object" === type ? key : prop.idx];
                null == key && (key = local[prop.idx]),
                local[prop.idx] = clamp(key, prop)
            }),
        ...
        each(props, function(key, prop) {
            color.fn[key] || (color.fn[key] = function(value) {
                var match, vtype = getType(value), fn = "alpha" === key ? this._hsla ? "hsla" : "rgba" : spaceName, local = this[fn](), cur = local[prop.idx];
        ...
        each(hook, function(_i, hook) {
            jQuery.cssHooks[hook] = {
                set: function(elem, value) {
                    ...
                    if ("transparent" !== value && ("string" !== getType(value) || (parsed = stringParse(value)))) {
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/effect", ["jquery", "./jquery-var-for-color", "./vendor/jquery-color/jquery.color", "./version"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    ...
    function getElementStyles(elem) {
        ...
        if (style && style.length && style[0] && style[style[0]])
            for (len = style.length; len--; )
                "string" == typeof style[key = style[len]] && (styles[key.replace(/-([\da-z])/gi, function(all, letter) {
    ...
    function() {
        ...
        function _normalizeArguments(effect, options, speed, callback) {
            ...
            speed = options,
            options = {}),
            "function" == typeof speed && (callback = speed,
            speed = null),
        ...
        function parseClip(str, element) {
            var outerWidth = element.outerWidth()
              , element = element.outerHeight()
              , str = /^rect\((-?\d*\.?\d*px|-?\d+%|auto),?\s*(-?\d*\.?\d*px|-?\d+%|auto),?\s*(-?\d*\.?\d*px|-?\d+%|auto),?\s*(-?\d*\.?\d*px|-?\d+%|auto)\)$/.exec(str) || ["", 0, outerWidth, element, 0];
            return {
        ...
        !1 !== $.uiBackCompat && $.extend($.effects, {
            ...
            restore: function(element, set) {
                for (var val, i = 0, length = set.length; i < length; i++)
                    null !== set[i] && (val = element.data("ui-effects-" + set[i]),
        ...
        $.extend($.effects, {
            ...
            createPlaceholder: function(element) {
                ...
                element.data("ui-effects-placeholder", placeholder)),
                element.css({
            ...
            removePlaceholder: function(element) {
                var dataKey = "ui-effects-placeholder"
        ...
        $.fn.extend({
            effect: function() {
                ...
                function run(next) {
                    ...
                    !1 === $.uiBackCompat || defaultMode ? "none" === args.mode ? (elem[mode](),
                    done()) : effectMethod.call(elem[0], args, function() {
                        elem.removeData("ui-effects-animated"),
            ...
            transfer: function(options, done) {
                var element = $(this)
                  ...
                  , body = targetFixed ? body.scrollLeft() : 0
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/effects/effect-blind.js", ["jquery", "../version", "../effect"], factory) : factory(jQuery)
}(function($) {
    "use strict";
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/effects/effect-bounce.js", ["jquery", "../version", "../effect"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.effects.define("bounce", function(options, done) {
        ...
        (downAnim = {})[ref] = direction; i < times; i++)
        ...
        element.queue(done),
        $.effects.unshift(element, options, 1 + anims)
    })
}),
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/effects/effect-clip.js", ["jquery", "../version", "../effect"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.effects.define("clip", "hide", function(options, done) {
        var animate = {}
          ...
          , both = "both" === direction
          , horizontal = both || "horizontal" === direction
          , both = both || "vertical" === direction
          , direction = element.cssClip();
        animate.clip = {
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/effects/effect-drop.js", ["jquery", "../version", "../effect"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.effects.define("drop", "hide", function(options, done) {
        ...
        animation[ref] = direction + distance,
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/effects/effect-explode.js", ["jquery", "../version", "../effect"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.effects.define("explode", "hide", function(options, done) {
        ...
        function childComplete() {
            pieces.push(this),
            pieces.length === rows * cells && (element.css({
                visibility: "visible"
            }),
            $(pieces).remove(),
            done())
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/effects/effect-fade.js", ["jquery", "../version", "../effect"], factory) : factory(jQuery)
}(function($) {
    "use strict";
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/effects/effect-fold.js", ["jquery", "../version", "../effect"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.effects.define("fold", "hide", function(options, done) {
        var element = $(this)
          ...
          , ref = !!options.horizFirst ? ["right", "bottom"] : ["bottom", "right"]
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/effects/effect-highlight.js", ["jquery", "../version", "../effect"], factory) : factory(jQuery)
}(function($) {
    "use strict";
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/effects/effect-size.js", ["jquery", "../version", "../effect"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.effects.define("size", function(options, done) {
        ...
        origin && (mode = $.effects.getBaseline(origin, original),
        ...
        to.top = (original.outerHeight - to.outerHeight) * mode.y + pos.top,
        to.left = (original.outerWidth - to.outerWidth) * mode.x + pos.left),
        ...
        element.find("*[width]").each(function() {
            ...
            child.css(childFrom),
            child.animate(childOriginal, options.duration, options.easing, function() {
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/effects/effect-scale.js", ["jquery", "../version", "../effect", "./effect-size"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.effects.define("scale", function(options, done) {
        var el = $(this)
          , mode = options.mode
          , mode = parseInt(options.percent, 10) || (0 === parseInt(options.percent, 10) || "effect" !== mode ? 0 : 100)
          , el = $.extend(!0, {
            from: $.effects.scaledDimensions(el),
            to: $.effects.scaledDimensions(el, mode, options.direction || "both"),
            origin: options.origin || ["middle", "center"]
        }, options);
        options.fade && (el.from.opacity = 1,
        el.to.opacity = 0),
        $.effects.effect.size.call(this, el, done)
    })
}),
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/effects/effect-puff.js", ["jquery", "../version", "../effect", "./effect-scale"], factory) : factory(jQuery)
}(function($) {
    "use strict";
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/effects/effect-pulsate.js", ["jquery", "../version", "../effect"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.effects.define("pulsate", "show", function(options, done) {
        var element = $(this)
          , mode = options.mode
          , show = "show" === mode
          , anims = 2 * (options.times || 5) + (show || "hide" === mode ? 1 : 0)
          , duration = options.duration / anims
          , animateTo = 0
          , i = 1
          , mode = element.queue().length;
        for (!show && element.is(":visible") || (element.css("opacity", 0).show(),
        animateTo = 1); i < anims; i++)
            element.animate({
                opacity: animateTo
        ...
        element.queue(done),
        $.effects.unshift(element, mode, 1 + anims)
    })
}),
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/effects/effect-shake.js", ["jquery", "../version", "../effect"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.effects.define("shake", function(options, done) {
        var i = 1
          ...
          , direction = "up" === direction || "left" === direction
          , animation = {}
          , animation1 = {}
          , animation2 = {}
          , queuelen = element.queue().length;
        for ($.effects.createPlaceholder(element),
        animation[ref] = (direction ? "-=" : "+=") + distance,
        animation1[ref] = (direction ? "+=" : "-=") + 2 * distance,
        animation2[ref] = (direction ? "-=" : "+=") + 2 * distance,
        element.animate(animation, speed, options.easing); i < times; i++)
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/effects/effect-slide.js", ["jquery", "../version", "../effect"], factory) : factory(jQuery)
}(function($) {
    "use strict";
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/effects/effect-transfer.js", ["jquery", "../version", "../effect"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    var effect;
    return effect = !1 !== $.uiBackCompat ? $.effects.define("transfer", function(options, done) {
        $(this).transfer(options, done)
    }) : effect
}),
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/focusable", ["jquery", "./version"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.ui.focusable = function(element, hasTabindex) {
        var mapName, map, focusableIfVisible, fieldset, nodeName = element.nodeName.toLowerCase();
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/form.js", ["jquery", "./version"], factory) : factory(jQuery)
}(function($) {
    "use strict";
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/form-reset-mixin", ["jquery", "./form", "./version"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.ui.formResetMixin = {
        ...
        _bindFormResetHandler: function() {
            ...
            this.form.length && ((instances = this.form.data("ui-form-reset-instances") || []).length || this.form.on("reset.ui-form-reset", this._formResetHandler),
            instances.push(this),
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/ie", ["jquery", "./version"], factory) : factory(jQuery)
}(function($) {
    "use strict";
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/jquery-patch.js", ["jquery", "./version"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    var rcssescape, fcssescape;
    $.expr.pseudos || ($.expr.pseudos = $.expr[":"]),
    $.uniqueSort || ($.uniqueSort = $.unique),
    $.escapeSelector || (rcssescape = /([\0-\x1f\x7f]|^-?\d)|^-$|[^\x80-\uFFFF\w-]/g,
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/keycode", ["jquery", "./version"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.ui.keyCode = {
        BACKSPACE: 8,
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/labels", ["jquery", "./version"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.fn.labels = function() {
        ...
        id = "label[for='" + $.escapeSelector(id) + "']",
        labels = labels.add(ancestor.find(id).addBack(id))),
        this.pushStack(labels)) : this.pushStack([])
    }
}),
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/plugin", ["jquery", "./version"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.ui.plugin = {
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/position", ["jquery", "./version"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    ...
    $.position = {
        scrollbarWidth: function() {
            ...
            var w1, div = $("<div style='display:block;position:absolute;width:200px;height:200px;overflow:hidden;'><div style='height:300px;width:auto;'></div></div>"), innerDiv = div.children()[0];
            return $("body").append(div),
            w1 = innerDiv.offsetWidth,
            div.css("overflow", "scroll"),
            w1 === (innerDiv = innerDiv.offsetWidth) && (innerDiv = div[0].clientWidth),
            div.remove(),
            cachedScrollbarWidth = w1 - innerDiv
        },
        getScrollInfo: function(within) {
            var overflowX = within.isWindow || within.isDocument ? "" : within.element.css("overflow-x")
              , overflowY = within.isWindow || within.isDocument ? "" : within.element.css("overflow-y")
              , overflowX = "scroll" === overflowX || "auto" === overflowX && within.width < within.element[0].scrollWidth;
            return {
    ...
    $.fn.position = function(options) {
        ...
        var atOffset, targetWidth, targetHeight, targetOffset, basePosition, elem, target = "string" == typeof (options = $.extend({}, options)).of ? $(document).find(options.of) : $(options.of), within = $.position.getWithinInfo(options.within), scrollInfo = $.position.getScrollInfo(within), collision = (options.collision || "flip").split(" "), offsets = {}, raw = 9 === (raw = (elem = target)[0]).nodeType ? {
            width: elem.width(),
            height: elem.height(),
            offset: {
                top: 0,
        ...
        targetHeight = raw.height,
        basePosition = $.extend({}, targetOffset = raw.offset),
        $.each(["my", "at"], function() {
        ...
        flip: {
            left: function(position, data) {
                var within = data.within
                  ...
                  , overLeft = collisionPosLeft - within
                  , collisionPosLeft = collisionPosLeft + data.collisionWidth - outerWidth - within
            ...
            top: function(position, data) {
                var within = data.within
                  ...
                  , overTop = collisionPosTop - within
                  , collisionPosTop = collisionPosTop + data.collisionHeight - outerHeight - within
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/safe-active-element", ["jquery", "./version"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.ui.safeActiveElement = function(document) {
        ...
        return activeElement = (activeElement = activeElement || document.body).nodeName ? activeElement : document.body
    }
}),
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/safe-blur", ["jquery", "./version"], factory) : factory(jQuery)
}(function($) {
    "use strict";
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/scroll-parent", ["jquery", "./version"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.fn.scrollParent = function(includeHidden) {
        var position = this.css("position")
          ...
          , includeHidden = this.parents().filter(function() {
            var parent = $(this);
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/tabbable", ["jquery", "./version", "./focusable"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.extend($.expr.pseudos, {
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/unique-id", ["jquery", "./version"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.fn.extend({
        uniqueId: (uuid = 0,
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/widget", ["jquery", "./version"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    var orig, widgetUuid = 0, widgetHasOwnProperty = Array.prototype.hasOwnProperty, widgetSlice = Array.prototype.slice;
    return $.cleanData = (orig = $.cleanData,
    function(elems) {
    ...
    $.widget = function(name, base, prototype) {
        ...
        base = $.Widget),
        Array.isArray(prototype) && (prototype = $.extend.apply(null, [{}].concat(prototype))),
        ...
        $.each(prototype, function(prop, value) {
            ...
            proxiedPrototype[prop] = "function" == typeof value ? function() {
    ...
    $.widget.bridge = function(name, object) {
        ...
        $.fn[name] = function(options) {
            ...
            return isMethodCall ? this.length || "instance" !== options ? this.each(function() {
                ...
                !1) : instance ? "function" != typeof instance[options] || "_" === options.charAt(0) ? $.error("no such method '" + options + "' for " + name + " widget instance") : (methodValue = instance[options].apply(instance, args)) !== instance && void 0 !== methodValue ? (returnValue = methodValue && methodValue.jquery ? returnValue.pushStack(methodValue.get()) : methodValue,
    ...
    $.Widget.prototype = {
        ...
        _toggleClass: function(element, keys, extra, add) {
            ...
            return keys.element.toggleClass(this._classes(keys), add),
            this
        },
        _on: function(suppressDisabledCheck, element, handlers) {
            ...
            $.each(handlers, function(event, handler) {
                ...
                var event = event.match(/^([\w:-]*)\s*(.*)$/)
                  , eventName = event[1] + instance.eventNamespace
                  , event = event[2];
        ...
        _trigger: function(type, event, data) {
            ...
            !("function" == typeof callback && !1 === callback.apply(this.element[0], [event].concat(data)) || event.isDefaultPrevented())
        }
    },
    $.each({
        show: "fadeIn",
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/widgets/accordion.js", ["jquery", "../version", "../keycode", "../unique-id", "../widget"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.widget("ui.accordion", {
        ...
        _createIcons: function() {
            ...
            icon = this.active.children(".ui-accordion-header-icon"),
        ...
        _setOption: function(key, value) {
            ...
            value && this._createIcons())) : this._activate(value)
        },
        _setOptionDisabled: function(value) {
            this._super(value),
        ...
        _activate: function(index) {
            ...
            index !== this.active[0] && (index = index || this.active[0],
            this._eventHandler({
                target: index,
                currentTarget: index,
                preventDefault: $.noop
        ...
        _eventHandler: function(event) {
            ...
            event.preventDefault(),
            clickedIsActive && !options.collapsible || !1 === this._trigger("beforeActivate", event, toHide) || (options.active = !collapsing && this.headers.index(clicked),
        ...
        _animate: function(toShow, toHide, data) {
            ...
            var total, easing, duration, that = this, adjust = 0, boxSizing = toShow.css("box-sizing"), down = toShow.length && (!toHide.length || toShow.index() < toHide.index()), animate = this.options.animate || {}, down = down && animate.down || animate;
            return easing = (easing = "string" == typeof down ? down : easing) || down.easing || animate.easing,
            duration = (duration = "number" == typeof down ? down : duration) || down.duration || animate.duration,
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/widgets/menu.js", ["jquery", "../keycode", "../position", "../safe-active-element", "../unique-id", "../version", "../widget"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.widget("ui.menu", {
        ...
        _keydown: function(event) {
            ...
            default:
                ...
                skip = preventDefault = !1,
                character = 96 <= event.keyCode && event.keyCode <= 105 ? (event.keyCode - 96).toString() : String.fromCharCode(event.keyCode),
                clearTimeout(this.filterTimer),
                character === prev ? skip = !0 : character = prev + character,
                prev = this._filterMenuItems(character),
                (prev = skip && -1 !== prev.index(this.active.next()) ? this.active.nextAll(".ui-menu-item") : prev).length || (character = String.fromCharCode(event.keyCode),
                prev = this._filterMenuItems(character)),
                prev.length ? (this.focus(event, prev),
        ...
        _setOption: function(key, value) {
            var icons;
            "icons" === key && (icons = this.element.find(".ui-menu-icon"),
        ...
        focus: function(event, item) {
            ...
            (focused = item.children(".ui-menu")).length && event && /^mouse/.test(event.type) && this._startOpening(focused),
        ...
        _scrollIntoView: function(item) {
            ...
            paddingTop = this.activeMenu.scrollTop(),
            elementHeight = this.activeMenu.height(),
            item = item.outerHeight(),
            borderTop < 0 ? this.activeMenu.scrollTop(paddingTop + borderTop) : elementHeight < borderTop + item && this.activeMenu.scrollTop(paddingTop + borderTop - elementHeight + item))
        },
        ...
        nextPage: function(event) {
            ...
            0 === $.fn.jquery.indexOf("3.2.") && (height += this.element[0].offsetHeight - this.element.outerHeight()),
        ...
        _filterMenuItems: function(character) {
            var character = character.replace(/[\-\[\]{}()*+?.,\\\^$|#\s]/g, "\\$&")
              , regex = new RegExp("^" + character,"i");
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/widgets/autocomplete.js", ["jquery", "./menu", "../keycode", "../position", "../safe-active-element", "../version", "../widget"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.widget("ui.autocomplete", {
        ...
        _create: function() {
            ...
            this.isMultiLine = isTextarea || !nodeName && this._isContentEditable(this.element),
        ...
        _initSource: function() {
            ...
            Array.isArray(this.options.source) ? (array = this.options.source,
        ...
        _move: function(direction, event) {
            if (this.menu.element.is(":visible"))
                return this.menu.isFirstItem() && /^previous/.test(direction) || this.menu.isLastItem() && /^next/.test(direction) ? (this.isMultiLine || this._value(this.term),
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/widgets/controlgroup.js", ["jquery", "../widget"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    ...
    return $.widget("ui.controlgroup", {
        ...
        _initWidgets: function() {
            ...
            $.each(this.options.items, function(widget, selector) {
                var labels, options = {};
                if (selector)
                    return "controlgroupLabel" === widget ? ((labels = that.element.find(selector)).each(function() {
                        var element = $(this);
                    ...
                    that.element.find(selector).each(function() {
                        ...
                        instanceOptions = element[widget]("widget"),
        ...
        _spinnerOptions: function(position) {
            ...
            return position.classes["ui-spinner-up"] = "",
            position.classes["ui-spinner-down"] = "",
            position
        },
        ...
        _setOption: function(key, value) {
            ...
            this._super(key, value),
            "disabled" !== key ? this.refresh() : this._callChildMethod(value ? "disable" : "enable")
        },
        refresh: function() {
            ...
            (children = this.options.onlyVisible ? children.filter(":visible") : children).length && ($.each(["first", "last"], function(index, value) {
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/widgets/checkboxradio.js", ["jquery", "../form-reset-mixin", "../labels", "../widget"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.widget("ui.checkboxradio", [$.ui.formResetMixin, {
        ...
        _getCreateOptions: function() {
            var labels, that = this, options = this._super() || {};
            ...
            null != (labels = this.element[0].disabled) && (options.disabled = labels),
            options
        },
        _create: function() {
        ...
        _setOption: function(key, value) {
            if ("label" !== key || value) {
                ...
                "disabled" === key)
                    return this._toggleClass(this.label, null, "ui-state-disabled", value),
        ...
        _updateLabel: function() {
            ...
            (contents = this.iconSpace ? contents.not(this.iconSpace[0]) : contents).remove(),
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/widgets/button", ["jquery", "./controlgroup", "./checkboxradio", "../keycode", "../widget"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    var orig;
    return $.widget("ui.button", {
        ...
        _getCreateOptions: function() {
            ...
            null != (disabled = this.element[0].disabled) && (options.disabled = disabled),
        ...
        _updateIcon: function(option, value) {
            var option = "iconPosition" !== option
              , position = option ? this.options.iconPosition : value
            ...
            option && this._addClass(this.icon, null, value),
        ...
        _setOptions: function(options) {
            var newShowLabel = (void 0 === options.showLabel ? this.options : options).showLabel
              , newIcon = (void 0 === options.icon ? this.options : options).icon;
    ...
    !1 !== $.uiBackCompat && ($.widget("ui.button", $.ui.button, {
        ...
        _setOption: function(key, value) {
            "text" !== key ? ("showLabel" === key && (this.options.text = value),
            ...
            this._superApply(arguments)) : this._super("showLabel", value)
        }
    }),
    $.fn.button = (orig = $.fn.button,
    function(options) {
        var isMethodCall = "string" == typeof options
        ...
        return isMethodCall ? this.length || "instance" !== options ? this.each(function() {
            ...
            return "instance" === options ? (returnValue = type,
            !1) : type ? "function" != typeof type[options] || "_" === options.charAt(0) ? $.error("no such method '" + options + "' for button widget instance") : (methodValue = type[options].apply(type, args)) !== type && void 0 !== methodValue ? (returnValue = methodValue && methodValue.jquery ? returnValue.pushStack(methodValue.get()) : methodValue,
            !1) : void 0 : $.error("cannot call methods on button prior to initialization; attempted to call method '" + options + "'")
        }) : returnValue = void 0 : (args.length && (options = $.widget.extend.apply(null, [options].concat(args))),
        this.each(function() {
            var type = $(this).attr("type")
              , type = "checkbox" !== type && "radio" !== type ? "button" : "checkboxradio"
              , instance = $.data(this, "ui-" + type);
            instance ? (instance.option(options || {}),
            instance._init && instance._init()) : "button" != type ? $(this).checkboxradio($.extend({
                icon: !1
            }, options)) : orig.call($(this), options)
        })),
        returnValue
    }
    ),
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/widgets/datepicker.js", ["jquery", "../version", "../keycode"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    ...
    $.extend(Datepicker.prototype, {
        ...
        _attachments: function(input, inst) {
            ...
            appendText && (inst.append = $("<span>").addClass(this._appendClass).text(appendText),
            input[isRTL ? "before" : "after"](inst.append)),
            input.off("focus", this._showDatepicker),
            inst.trigger && inst.trigger.remove(),
            "focus" !== (appendText = this._get(inst, "showOn")) && "both" !== appendText || input.on("focus", this._showDatepicker),
            "button" !== appendText && "both" !== appendText || (appendText = this._get(inst, "buttonText"),
            buttonImage = this._get(inst, "buttonImage"),
            this._get(inst, "buttonImageOnly") ? inst.trigger = $("<img>").addClass(this._triggerClass).attr({
                src: buttonImage,
                alt: appendText,
                title: appendText
        ...
        _dialogDatepicker: function(input, date, onSelect, settings, pos) {
            ...
            this._pos || (id = document.documentElement.clientWidth,
            settings = document.documentElement.clientHeight,
            date = document.documentElement.scrollLeft || document.body.scrollLeft,
            pos = document.documentElement.scrollTop || document.body.scrollTop,
            this._pos = [id / 2 - 100 + date, settings / 2 - 150 + pos]),
        ...
        _optionDatepicker: function(target, name, value) {
            ...
            name = this._getDateDatepicker(target, !0),
            value = this._getMinMaxDate(inst, "min"),
            maxDate = this._getMinMaxDate(inst, "max"),
            datepicker_extendRemove(inst.settings, settings),
            null !== value && void 0 !== settings.dateFormat && void 0 === settings.minDate && (inst.settings.minDate = this._formatDate(inst, value)),
        ...
        _setDateDatepicker: function(target, date) {
            target = this._getInst(target);
            target && (this._setDate(target, date),
            this._updateDatepicker(target),
        ...
        _doKeyPress: function(event) {
            ...
            if ($.datepicker._get(inst, "constrainInput"))
                return inst = $.datepicker._possibleChars($.datepicker._get(inst, "dateFormat")),
        ...
        _doKeyUp: function(event) {
            ...
            if (event.input.val() !== event.lastVal)
                try {
                    ...
                    $.datepicker._updateAlternate(event),
                    $.datepicker._updateDatepicker(event))
                } catch (err) {}
            return !0
        },
        _showDatepicker: function(input) {
            ...
            beforeShow = $.datepicker._checkOffset(inst, beforeShow, isFixed),
            inst.dpDiv.css({
            ...
            $.effects && $.effects.effect[beforeShow] ? inst.dpDiv.show(beforeShow, $.datepicker._get(inst, "showOptions"), duration) : inst.dpDiv[beforeShow || "show"](beforeShow ? duration : null),
        ...
        _updateDatepicker: function(inst) {
            ...
            onUpdateDatepicker && onUpdateDatepicker.apply(inst.input ? inst.input[0] : null, [inst])
        },
        ...
        _findPos: function(obj) {
            for (var inst = this._getInst(obj), isRTL = this._get(inst, "isRTL"); obj && ("hidden" === obj.type || 1 !== obj.nodeType || $.expr.pseudos.hidden(obj)); )
        ...
        _hideDatepicker: function(input) {
            ...
            ,
            $.effects && ($.effects.effect[input] || $.effects[input]) ? inst.dpDiv.hide(input, $.datepicker._get(inst, "showOptions"), duration, postProcess) : inst.dpDiv["slideDown" === input ? "slideUp" : "fadeIn" === input ? "fadeOut" : "hide"](input ? duration : null, postProcess),
            input || postProcess(),
            this._datepickerShowing = !1,
            (duration = this._get(inst, "onClose")) && duration.apply(inst.input ? inst.input[0] : null, [inst.input ? inst.input.val() : "", inst]),
        ...
        _checkExternalClick: function(event) {
            var inst;
            $.datepicker._curInst && (event = $(event.target),
            inst = $.datepicker._getInst(event[0]),
        ...
        _selectDay: function(id, month, year, td) {
            ...
            target.selectedMonth = target.currentMonth = month,
            target.selectedYear = target.currentYear = year,
        ...
        _selectDate: function(id, dateStr) {
            ...
            dateStr = null != dateStr ? dateStr : this._formatDate(id),
            id.input && id.input.val(dateStr),
            this._updateAlternate(id),
            (onSelect = this._get(id, "onSelect")) ? onSelect.apply(id.input ? id.input[0] : null, [dateStr, id]) : id.input && id.input.trigger("change"),
            id.inline ? this._updateDatepicker(id) : (this._hideDatepicker(),
            this._lastInput = id.input[0],
            "object" != typeof id.input[0] && id.input.trigger("focus"),
            this._lastInput = null)
        },
        _updateAlternate: function(inst) {
            ...
            altFormat = this.formatDate(altFormat, date, this._getFormatConfig(inst)),
        ...
        iso8601Week: function(date) {
            ...
            return date.setDate(date.getDate() + 4 - (date.getDay() || 7)),
            time = date.getTime(),
            date.setMonth(0),
            date.setDate(1),
            Math.floor(Math.round((time - date) / 864e5) / 7) + 1
        },
        ...
        _generateHTML: function(inst) {
            ...
            showCurrentAtPos = this._get(inst, "prevText"),
            ...
            showCurrentAtPos = this._get(inst, "nextText"),
            showCurrentAtPos = navigationAsDateFormat ? this.formatDate(showCurrentAtPos, this._daylightSavingAdjust(new Date(drawYear,drawMonth + stepMonths,1)), this._getFormatConfig(inst)) : showCurrentAtPos,
            ...
            stepMonths = this._get(inst, "currentText"),
            hideIfNoPrevNext = this._get(inst, "gotoCurrent") && inst.currentDay ? currentDate : today,
            stepMonths = navigationAsDateFormat ? this.formatDate(stepMonths, hideIfNoPrevNext, this._getFormatConfig(inst)) : stepMonths,
            showCurrentAtPos = "",
            inst.inline || (showCurrentAtPos = $("<button>").attr({
                type: "button",
            ...
            row = 0; row < numMonths[0]; row++) {
                ...
                col = 0; col < numMonths[1]; col++) {
                    ...
                    numRows = isMultiMonth && this.maxRows > daysInMonth ? this.maxRows : daysInMonth,
            ...
            return html += navigationAsDateFormat,
            inst._keyEvent = !1,
            html
        },
        _generateMonthYearHeader: function(inst, drawMonth, drawYear, minDate, maxDate, secondary, monthNames, monthNamesShort) {
            ...
            !inst.yearshtml)
                ...
                secondary || !changeYear)
                    ...
                    for (monthNames = this._get(inst, "yearRange").split(":"),
                    ...
                    endYear = Math.max(year, selectMonthLabel(monthNames[1] || "")),
                    year = minDate ? Math.max(year, minDate.getFullYear()) : year,
                    endYear = maxDate ? Math.min(endYear, maxDate.getFullYear()) : endYear,
                    inst.yearshtml += "<select class='ui-datepicker-year' aria-label='" + selectYearLabel + "' data-handler='selectYear' data-event='change'>"; year <= endYear; year++)
        ...
        _canAdjustMonth: function(inst, offset, curYear, curMonth) {
            var numMonths = this._getNumberOfMonths(inst)
              , curYear = this._daylightSavingAdjust(new Date(curYear,curMonth + (offset < 0 ? offset : numMonths[0] * numMonths[1]),1));
        ...
        _isInRange: function(inst, date) {
            ...
            return inst && (inst = inst.split(":"),
            currentYear = (new Date).getFullYear(),
            minYear = parseInt(inst[0], 10),
            maxYear = parseInt(inst[1], 10),
            inst[0].match(/[+\-].*/) && (minYear += currentYear),
            inst[1].match(/[+\-].*/) && (maxYear += currentYear)),
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/widgets/mouse.js", ["jquery", "../ie", "../version", "../widget"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    var mouseHandled = !1;
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/widgets/draggable.js", ["jquery", "./mouse", "../data", "../plugin", "../safe-active-element", "../safe-blur", "../scroll-parent", "../version", "../widget"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.widget("ui.draggable", $.ui.mouse, {
        ...
        _mouseDrag: function(event, noPropagation) {
            ...
            !noPropagation) {
                ...
                if (!1 === this._trigger("drag", event, noPropagation))
        ...
        _mouseStop: function(event) {
            ...
            this.dropped = !1),
            "invalid" === this.options.revert && !dropped || "valid" === this.options.revert && dropped || !0 === this.options.revert || "function" == typeof this.options.revert && this.options.revert.call(this.element, dropped) ? $(this.helper).animate(this.originalPosition, parseInt(this.options.revertDuration, 10), function() {
        ...
        _createHelper: function(event) {
            var o = this.options
              , helperIsFunction = "function" == typeof o.helper
              , event = helperIsFunction ? $(o.helper.apply(this.element[0], [event])) : "clone" === o.helper ? this.element.clone().removeAttr("id") : this.element;
        ...
        _generatePosition: function(event, constrainPosition) {
            ...
            constrainPosition && (this.containment && (containment = this.relativeContainer ? (constrainPosition = this.relativeContainer.offset(),
        ...
        drag: function(event, ui, draggable) {
            $.each(draggable.sortables, function() {
                ...
                $.each(draggable.sortables, function() {
                    ...
                    innermostIntersecting = this !== sortable && this._intersectsWith(this.containerCache) && $.contains(sortable.element[0], this.element[0]) ? !1 : innermostIntersecting
                })),
    ...
    $.ui.plugin.add("draggable", "cursor", {
        start: function(event, ui, instance) {
            ...
            t.css("cursor") && (instance._cursor = t.css("cursor")),
            t.css("cursor", instance.cursor)
        },
    ...
    $.ui.plugin.add("draggable", "opacity", {
        start: function(event, ui, instance) {
            ui = $(ui.helper),
            instance = instance.options;
            ui.css("opacity") && (instance._opacity = ui.css("opacity")),
            ui.css("opacity", instance.opacity)
        },
        stop: function(event, ui, instance) {
            instance = instance.options;
    ...
    $.ui.plugin.add("draggable", "snap", {
        ...
        drag: function(event, ui, inst) {
            for (var ts, bs, ls, rs, l, r, t, b, first, o = inst.options, d = o.snapTolerance, x1 = ui.offset.left, x2 = x1 + inst.helperProportions.width, y1 = ui.offset.top, y2 = y1 + inst.helperProportions.height, i = inst.snapElements.length - 1; 0 <= i; i--)
                r = (l = inst.snapElements[i].left - inst.margins.left) + inst.snapElements[i].width,
    ...
    $.ui.plugin.add("draggable", "zIndex", {
        start: function(event, ui, instance) {
            ui = $(ui.helper),
            instance = instance.options;
            ui.css("zIndex") && (instance._zIndex = ui.css("zIndex")),
            ui.css("zIndex", instance.zIndex)
        },
        stop: function(event, ui, instance) {
            instance = instance.options;
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/widgets/resizable.js", ["jquery", "./mouse", "../disable-selection", "../plugin", "../version", "../widget"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.widget("ui.resizable", $.ui.mouse, {
        ...
        _hasScroll: function(el, a) {
            if ("hidden" === $(el).css("overflow"))
                return !1;
            var a = a && "left" === a ? "scrollLeft" : "scrollTop"
              , has = !1;
            if (0 < el[a])
                return !0;
            try {
        ...
        _setupHandles: function() {
            ...
            this.handles.constructor === String)
                ...
                this.handles = {},
                i = 0; i < n.length; i++)
                    hname = "ui-resizable-" + (handle = String.prototype.trim.call(n[i])),
                    axis = $("<div>"),
                    ...
                    this.element.children(this.handles[handle]).length || (this.element.append(axis),
        ...
        _mouseStart: function(event) {
            ...
            el = $(".ui-resizable-" + this.axis).css("cursor"),
            $("body").css("cursor", "auto" === el ? this.axis + "-resize" : el),
        ...
        _mouseDrag: function(event) {
            ...
            a && (a = a.apply(this, [event, dx, smp]),
            this._updateVirtualBoundaries(event.shiftKey),
            (this._aspectRatio || event.shiftKey) && (a = this._updateRatio(a, event)),
            a = this._respectSize(a, event),
            this._updateCache(a),
            this._propagate("resize", event),
            dx = this._applyChanges(),
            !this._helper && this._proportionallyResizeElements.length && this._proportionallyResize(),
            $.isEmptyObject(dx) || (this._updatePrevProperties(),
        ...
        _mouseStop: function(event) {
            ...
            ista = ista ? 0 : this.sizeDiff.width,
            ista = {
        ...
        _updateVirtualBoundaries: function(forceAspectRatio) {
            ...
            pMinHeight = o.minWidth / this.aspectRatio,
            pMaxWidth = o.maxHeight * this.aspectRatio,
            pMaxHeight = o.maxWidth / this.aspectRatio,
            forceAspectRatio > o.minWidth && (o.minWidth = forceAspectRatio),
            pMinHeight > o.minHeight && (o.minHeight = pMinHeight),
            pMaxWidth < o.maxWidth && (o.maxWidth = pMaxWidth),
            pMaxHeight < o.maxHeight && (o.maxHeight = pMaxHeight)),
            this._vBoundaries = o
        },
        ...
        _respectSize: function(data) {
            ...
            isminh && a && (data.top = dh - o.minHeight),
            ismaxh && a && (data.top = dh - o.maxHeight),
        ...
        _getPaddingPlusBorderDimensions: function(element) {
            for (var i = 0, widths = [], borders = [element.css("borderTopWidth"), element.css("borderRightWidth"), element.css("borderBottomWidth"), element.css("borderLeftWidth")], paddings = [element.css("paddingTop"), element.css("paddingRight"), element.css("paddingBottom"), element.css("paddingLeft")]; i < 4; i++)
        ...
        _renderProxy: function() {
            ...
            this._helper ? (this.helper = this.helper || $("<div></div>").css({
                overflow: "hidden"
            }),
    ...
    $.ui.plugin.add("resizable", "animate", {
        stop: function(event) {
            var that = $(this).resizable("instance")
              ...
              , ista = ista ? 0 : that.sizeDiff.width
              , ista = {
                width: that.size.width - ista,
                height: that.size.height - soffseth
            }
              , soffseth = parseFloat(that.element.css("left")) + (that.position.left - that.originalPosition.left) || null
    ...
    $.ui.plugin.add("resizable", "containment", {
        start: function() {
            ...
            el && (that.containerElement = $(el),
            /document/.test(o) || o === document ? (that.containerOffset = {
                left: 0,
                top: 0
            },
            ...
            o = that.containerOffset,
            ch = that.containerSize.height,
            cw = that.containerSize.width,
            cw = that._hasScroll(el, "left") ? el.scrollWidth : cw,
            ch = that._hasScroll(el) ? el.scrollHeight : ch,
            that.parentData = {
                element: el,
                left: o.left,
                top: o.top,
        ...
        resize: function(event) {
            ...
            event && (that.size.height = that.size.width / that.aspectRatio,
            continueResize = !1),
            ...
            event && (that.size.width = that.size.height * that.aspectRatio,
            continueResize = !1),
            that.position.top = that._helper ? co.top : 0),
            ce = that.containerElement.get(0) === that.element.parent().get(0),
            ...
            cp = Math.abs(that.sizeDiff.width + (that._helper ? that.offset.left - cop.left : that.offset.left - co.left)),
        ...
        stop: function() {
            var that = $(this).resizable("instance")
              ...
              , helper = helper.outerHeight() - that.sizeDiff.height;
    ...
    $.ui.plugin.add("resizable", "ghost", {
        start: function() {
            ...
            !1 !== $.uiBackCompat && "string" == typeof that.options.ghost && that.ghost.addClass(this.options.ghost),
    ...
    $.ui.plugin.add("resizable", "grid", {
        resize: function() {
            ...
            that.position.top = op.top - cs) : /^(sw)$/.test(a) ? (that.size.width = newWidth,
            that.size.height = newHeight,
            that.position.left = op.left - ox) : ((newHeight - gridY <= 0 || newWidth - gridX <= 0) && (outerDimensions = that._getPaddingPlusBorderDimensions(this)),
            0 < newHeight - gridY ? (that.size.height = newHeight,
            that.position.top = op.top - cs) : (newHeight = gridY - outerDimensions.height,
            that.size.height = newHeight,
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/widgets/dialog.js", ["jquery", "./button", "./draggable", "./mouse", "./resizable", "../focusable", "../keycode", "../position", "../safe-active-element", "../safe-blur", "../tabbable", "../unique-id", "../version", "../widget"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.widget("ui.dialog", {
        ...
        _moveToTop: function(event, silent) {
            var moved = !1
              ...
              , zIndices = Math.max.apply(null, zIndices);
        ...
        _createWrapper: function() {
            ...
            this._on(this.uiDialog, {
                keydown: function(event) {
                    ...
                    event.keyCode !== $.ui.keyCode.TAB || event.isDefaultPrevented() || (tabbables = this.uiDialog.find(":tabbable"),
        ...
        _createButtons: function() {
            ...
            $.isEmptyObject(buttons) || Array.isArray(buttons) && !buttons.length ? this._removeClass(this.uiDialog, "ui-dialog-buttons") : ($.each(buttons, function(name, props) {
                var click;
                props = $.extend({
                    type: "button"
            ...
            this.uiDialog.resizable({
                ...
                stop: function(event, ui) {
                    var offset = that.uiDialog.offset()
                      ...
                      , offset = offset.top - that.document.scrollTop();
        ...
        _setOption: function(key, value) {
            ...
            isDraggable || !1 === value || this._makeResizable()),
        ...
        _createOverlay: function() {
            var jqMinor, isOpening;
            this.options.modal && (jqMinor = $.fn.jquery.substring(0, 4),
            isOpening = !0,
            this._delay(function() {
                isOpening = !1
            }),
            this.document.data("ui-dialog-overlays") || this.document.on("focusin.ui-dialog", function(event) {
                ...
                "3.4." !== jqMinor && "3.5." !== jqMinor || instance._delay(instance._restoreTabbableFocus)))
            }
            .bind(this)),
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/widgets/droppable.js", ["jquery", "./draggable", "./mouse", "../version", "../widget"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    ...
    return $.widget("ui.droppable", {
        ...
        _create: function() {
            ...
            this.isover = !1,
            this.isout = !0,
            this.accept = "function" == typeof accept ? accept : function(d) {
                return d.is(accept)
            }
            ,
    ...
    !($.ui.ddmanager = {
        ...
        drag: function(draggable, event) {
            ...
            $.each($.ui.ddmanager.droppables[draggable.options.scope] || [], function() {
                ...
                this[intersects] = !0,
                this["isout" === intersects ? "isover" : "isout"] = !1,
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/widgets/progressbar.js", ["jquery", "../version", "../widget"], factory) : factory(jQuery)
}(function($) {
    "use strict";
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/widgets/selectable.js", ["jquery", "./mouse", "../version", "../widget"], factory) : factory(jQuery)
}(function($) {
    "use strict";
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/widgets/selectmenu.js", ["jquery", "./menu", "../form-reset-mixin", "../keycode", "../labels", "../position", "../unique-id", "../version", "../widget"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.widget("ui.selectmenu", [$.ui.formResetMixin, {
        ...
        _drawMenu: function() {
            ...
            this.menuInstance = this.menu.menu({
                ...
                focus: function(event, ui) {
                    ...
                    that.isOpen || that._select(ui, event)),
                    that.focusIndex = ui.index,
        ...
        _refreshMenu: function() {
            ...
            this._rendered = !0,
            options.length && (options = this._getSelectedItem(),
            this.menuInstance.focus(null, options),
            this._setAria(options.data("ui-selectmenu-item")),
        ...
        _move: function(direction, event) {
            ...
            (item = "first" === direction || "last" === direction ? item["first" === direction ? "prevAll" : "nextAll"](filter).eq(-1) : item[direction + "All"](filter).eq(0)).length && this.menuInstance.focus(event, item)
        },
        ...
        _documentClick: {
            mousedown: function(event) {
                this.isOpen && ($(event.target).closest(".ui-selectmenu-menu, #" + $.escapeSelector(this.ids.button)).length || this.close(event))
            }
        },
        _buttonEvents: {
            mousedown: function() {
        ...
        _setOption: function(key, value) {
            var icon;
            "icons" === key && (icon = this.button.find("span.ui-icon"),
        ...
        _resizeButton: function() {
            ...
            !1 !== width ? (null === width && (width = this.element.show().outerWidth(),
            this.element.hide()),
        ...
        _parseOptions: function(options) {
            ...
            options.each(function(index, item) {
                item.hidden || data.push(that._parseOption($(item), index))
            }),
            this.items = data
        },
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/widgets/slider.js", ["jquery", "./mouse", "../keycode", "../version", "../widget"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.widget("ui.slider", $.ui.mouse, {
        ...
        _mouseDrag: function(event) {
            ...
            return this._slide(event, this._handleIndex, position),
            !1
        },
        _mouseStop: function(event) {
        ...
        _normValueFromMouse: function(position) {
            ...
            "vertical" === this.orientation && (position = 1 - position),
            pixelTotal = this._valueMax() - this._valueMin(),
            position = this._valueMin() + position * pixelTotal,
            this._trimAlignValue(position)
        },
        ...
        _setOption: function(key, value) {
            ...
            Array.isArray(this.options.values) && (valsLength = this.options.values.length),
            this._super(key, value),
            key) {
            case "orientation":
                this._detectOrientation(),
        ...
        _values: function(index) {
            var vals, i;
            if (arguments.length)
                return index = this.options.values[index],
                this._trimAlignValue(index);
        ...
        _trimAlignValue: function(val) {
            ...
            var step = 0 < this.options.step ? this.options.step : 1
              ...
              , val = val - valModStep;
            return 2 * Math.abs(valModStep) >= step && (val += 0 < valModStep ? step : -step),
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/widgets/sortable.js", ["jquery", "./mouse", "../data", "../ie", "../scroll-parent", "../version", "../widget"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.widget("ui.sortable", $.ui.mouse, {
        ...
        _mouseStart: function(event, overrideHandle, noActivation) {
            ...
            this.appendTo = $("parent" !== o.appendTo ? o.appendTo : this.currentItem.parent()),
        ...
        _intersectsWith: function(item) {
            var x1 = this.positionAbs.left
              ...
              , dyClick = "x" === this.options.axis || t < y1 + dyClick && y1 + dyClick < b
              , dxClick = "y" === this.options.axis || l < x1 + dxClick && x1 + dxClick < r;
        ...
        _intersectsWithPointer: function(item) {
            var isOverElementHeight = "x" === this.options.axis || this._isOverAxis(this.positionAbs.top + this.offset.click.top, item.top, item.height)
              , item = "y" === this.options.axis || this._isOverAxis(this.positionAbs.left + this.offset.click.left, item.left, item.width);
            return !(!isOverElementHeight || !item) && (isOverElementHeight = this.dragDirection.vertical,
            item = this.dragDirection.horizontal,
            this.floating ? "right" === item || "down" === isOverElementHeight ? 2 : 1 : isOverElementHeight && ("down" === isOverElementHeight ? 2 : 1))
        },
        _intersectsWithSides: function(item) {
            var isOverBottomHalf = this._isOverAxis(this.positionAbs.top + this.offset.click.top, item.top + item.height / 2, item.height)
              , item = this._isOverAxis(this.positionAbs.left + this.offset.click.left, item.left + item.width / 2, item.width)
        ...
        _getItemsAsjQuery: function(connected) {
            ...
            if (connectWith && connected)
                for (i = connectWith.length - 1; 0 <= i; i--)
                    for (j = (cur = $(connectWith[i], this.document[0])).length - 1; 0 <= j; j--)
                        (inst = $.data(cur[j], this.widgetFullName)) && inst !== this && !inst.options.disabled && queries.push(["function" == typeof inst.options.items ? inst.options.items.call(inst.element) : $(inst.options.items, inst.element).not(".ui-sortable-helper").not(".ui-sortable-placeholder"), inst]);
            function addItems() {
                items.push(this)
            }
        ...
        _refreshItems: function(event) {
            ...
            if (connectWith && this.ready)
                for (i = connectWith.length - 1; 0 <= i; i--)
                    for (j = (cur = $(connectWith[i], this.document[0])).length - 1; 0 <= j; j--)
                        (inst = $.data(cur[j], this.widgetFullName)) && inst !== this && !inst.options.disabled && (queries.push(["function" == typeof inst.options.items ? inst.options.items.call(inst.element[0], event, {
                            item: this.currentItem
        ...
        _refreshItemPositions: function(fast) {
            for (var item, t, i = this.items.length - 1; 0 <= i; i--)
                item = this.items[i],
        ...
        _contactContainers: function(event) {
            for (var j, dist, itemWithLeastDistance, posProperty, sizeProperty, cur, nearBottom, floating, axis, innermostContainer = null, innermostIndex = null, i = this.containers.length - 1; 0 <= i; i--)
            ...
            if (innermostContainer)
                if (1 === this.containers.length)
                    ...
                    posProperty = (floating = innermostContainer.floating || this._isFloating(this.currentItem)) ? "left" : "top",
                    ...
                    this.containers[innermostIndex].containerCache.over = 1) : this.currentContainer.containerCache.over || (this.containers[innermostIndex]._trigger("over", event, this._uiHash()),
                    this.currentContainer.containerCache.over = 1))
                }
        },
        _createHelper: function(event) {
            var o = this.options
              , event = "function" == typeof o.helper ? $(o.helper.apply(this.element[0], [event, this.currentItem])) : "clone" === o.helper ? this.currentItem.clone() : this.currentItem;
        ...
        _getParentOffset: function() {
            ...
            po.top += this.scrollParent.scrollTop()),
            {
                top: (po = this.offsetParent[0] === this.document[0].body || this.offsetParent[0].tagName && "html" === this.offsetParent[0].tagName.toLowerCase() && $.ui.ie ? {
                    top: 0,
                    left: 0
                } : po).top + (parseInt(this.offsetParent.css("borderTopWidth"), 10) || 0),
                left: po.left + (parseInt(this.offsetParent.css("borderLeftWidth"), 10) || 0)
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/widgets/spinner.js", ["jquery", "./button", "../version", "../keycode", "../safe-active-element", "../widget"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    ...
    return $.widget("ui.spinner", {
        ...
        _setOption: function(key, value) {
            ...
            if ("culture" === key || "numberFormat" === key)
                return prevValue = this._parse(this.element.val()),
                this.options[key] = value,
            ...
            this._removeClass(prevValue, null, this.options.icons.up),
            this._addClass(prevValue, null, value.up),
            prevValue = this.buttons.last().find(".ui-icon"),
            this._removeClass(prevValue, null, this.options.icons.down),
            this._addClass(prevValue, null, value.down)),
            this._super(key, value)
        },
        _setOptionDisabled: function(value) {
            this._super(value),
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/widgets/tabs.js", ["jquery", "../keycode", "../safe-active-element", "../unique-id", "../version", "../widget"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    var rhash;
    return $.widget("ui.tabs", {
        ...
        _isLocal: (rhash = /#.*$/,
        function(anchor) {
            var anchorUrl = anchor.href.replace(rhash, "")
        ...
        _create: function() {
            ...
            Array.isArray(options.disabled) && (options.disabled = $.uniqueSort(options.disabled.concat($.map(this.tabs.filter(".ui-state-disabled"), function(li) {
        ...
        _setOption: function(key, value) {
            "active" !== key ? (this._super(key, value),
        ...
        _setOptionDisabled: function(disabled) {
            ...
            i = 0; li = this.tabs[i]; i++)
                li = $(li),
                !0 === disabled || -1 !== $.inArray(i, disabled) ? (li.attr("aria-disabled", "true"),
                this._addClass(li, null, "ui-state-disabled")) : (li.removeAttr("aria-disabled"),
                this._removeClass(li, null, "ui-state-disabled"));
            this.options.disabled = disabled,
            this._toggleClass(this.widget(), this.widgetFullName + "-disabled", null, !0 === disabled)
        },
        _setupEvents: function(event) {
            var events = {};
            event && $.each(event.split(" "), function(index, eventName) {
        ...
        _eventHandler: function(event) {
            ...
            event.preventDefault(),
            tab.hasClass("ui-state-disabled") || tab.hasClass("ui-tabs-loading") || this.running || clickedIsActive && !options.collapsible || !1 === this._trigger("beforeActivate", event, active) || (options.active = !collapsing && this.tabs.index(tab),
        ...
        _getIndex: function(index) {
            return index = "string" == typeof index ? this.anchors.index(this.anchors.filter("[href$='" + $.escapeSelector(index) + "']")) : index
        },
        _destroy: function() {
            this.xhr && this.xhr.abort(),
...
function(factory) {
    "use strict";
    "function" == typeof define && define.amd ? define("ui/widgets/tooltip.js", ["jquery", "../keycode", "../position", "../unique-id", "../version", "../widget"], factory) : factory(jQuery)
}(function($) {
    "use strict";
    return $.widget("ui.tooltip", {
        ...
        _removeDescribedBy: function(elem) {
            var id = elem.data("ui-tooltip-id")
              ...
              , id = $.inArray(id, describedby);
            -1 !== id && describedby.splice(id, 1),
        ...
        _updateContent: function(target, event) {
            ...
            if ("string" == typeof contentOption || contentOption.nodeType || contentOption.jquery)
        ...
        _open: function(event, target, content) {
            ...
            (tooltipData = $("<div>").html(tooltip.find(".ui-tooltip-content").html())).removeAttr("name").find("[name]").removeAttr("name"),
            tooltipData.removeAttr("id").find("[id]").removeAttr("id"),
            tooltipData.appendTo(this.liveRegion),
        ...
        close: function(event) {
            ...
            tooltipData.closing || (clearInterval(this.delayedShow),
        ...
        _destroy: function() {
            ...
            $.each(this.tooltips, function(id, tooltipData) {
                var event = $.Event("blur")
                  , tooltipData = tooltipData.element;
                event.target = event.currentTarget = tooltipData[0],
                that.close(event, !0),
                $("#" + id).remove(),
                tooltipData.data("ui-tooltip-title") && (tooltipData.attr("title") || tooltipData.attr("title", tooltipData.data("ui-tooltip-title")),
